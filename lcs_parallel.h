...
template <bool onethread = false>
size_t ParallelLCS(size_t n, const auto& arrows) {
  const size_t inf = std::numeric_limits<size_t>::max();
  const size_t granularity = 5000;

  // parlay::sequence<std::pair<size_t, size_t>> c(n + m);
  // parlay::parallel_for(0, n, [&](size_t i) { c[i] = {a[i + 1], i + 1 + m};
  // }); parlay::parallel_for(0, m, [&](size_t i) { c[i + n] = {b[i + 1], i +
  // 1}; }); auto cg = parlay::group_by_key(c); parlay::sequence<size_t> go(n +
  // 1); parlay::parallel_for(0, cg.size(), [&](size_t i) {
  //   auto& [key, vals] = cg[i];
  ...
  std::function<void(size_t, size_t, size_t)> Construct =
      [&](size_t x, size_t l, size_t r) {
        ...
        size_t mid = (l + r) / 2;
        bool parallel = (!onethread) && r - l > granularity;
        conditional_par_do(
